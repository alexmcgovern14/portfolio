...
export function PRDSection({ project, viewMode, copied, onCopyPRD, onViewModeChange, onImageClick }: PRDSectionProps) {
  ...
  const markdownComponents = {
    ...
    img: ({ src, alt }: { src?: string; alt?: string }) => (
      <span className="block my-8">
        <span className="block rounded-[24px] p-[2px] overflow-hidden cursor-pointer" onClick={() => onImageClick && onImageClick(src || '')}
          style={{
        ...
        >
          <div className="px-4 md:px-8 pt-4 md:pt-8 pb-4 md:pb-6" style={{ borderBottom: '1px solid #8a8686' }}>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="inline-flex rounded-lg p-1 bg-[#4a4647]" style={{ border: '1px solid #8a8686' }}>
                  <button
